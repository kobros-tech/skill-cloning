...
def run_condition(target: str, prior_tasks: tuple[str, ...], history_label: str,
                  seed: int, condition_index: int):
    """Run one target/history/seed condition through the acquisition controller."""
    skills, prior_rows, history_valid = acquire_prior_skills(prior_tasks, seed, condition_index)
    ...
    if not history_valid:
        ...
        # partial history and do not expose an unsuccessful prerequisite to the controller.
        row.update({
    ...
    X_target, y_target = sample_task(target, N_TRAIN, seed=target_train_seed)
    ...
    for (target, history), group in raw.groupby(
        ...
        rows.append({
            ...
            "acquisition_success_rate": (
                attempted["acquisition_success"].mean() if len(attempted) else np.nan
            ),
            "mean_adaptation_steps": (
                attempted["adaptation_steps"].mean() if len(attempted) else np.nan
            ),
            "std_adaptation_steps": (
                attempted["adaptation_steps"].std() if len(attempted) else np.nan
            ),
            "mean_heldout_accuracy": (
                attempted["heldout_accuracy"].mean() if len(attempted) else np.nan
            ),
            "std_heldout_accuracy": (
                attempted["heldout_accuracy"].std() if len(attempted) else np.nan
            ),
            "mean_heldout_mse": (
                attempted["heldout_mse"].mean() if len(attempted) else np.nan
            ),
            "std_heldout_mse": (
                attempted["heldout_mse"].std() if len(attempted) else np.nan
            ),
...
def make_plot(summary: pd.DataFrame, out_path: Path):
    ...
    ax.bar(range(len(summary)), summary["acquisition_success_rate"].fillna(0.0))
...
def main():
    ...
    make_plot(summary, out_dir / "plot_prerequisite_acquisition.png")

    print("Prerequisite-based new-skill acquisition experiment")
    print(f"Seeds per condition: {N_SEEDS}; conditions: {len(CONDITIONS)}")
    print()
