...
def acquire_prior_skills(prior_tasks, seed: int, condition_index: int):
    ...
    for task in prior_tasks:
        ...
        skill = Skill(task, net, origin="scratch", parent=None)
        skills[task] = skill
        prior_rows.append({
            "prior_task": task,
...
def run_condition(target: str, prior_tasks: tuple[str, ...], history_label: str,
    ...
    skills, prior_rows = acquire_prior_skills(prior_tasks, seed, condition_index)
...
def summarize(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    for target, history, group in raw.groupby(
        ...
        rows.append({
            ...
            "std_heldout_accuracy": group["heldout_accuracy"].std(),
            "mean_heldout_mse": group["heldout_mse"].mean(),
            "std_heldout_mse": group["heldout_mse"].std(),
