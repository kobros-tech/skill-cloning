...
def summarize(raw: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for (target, history), group in raw.groupby(["target_task", "prior_history"], sort=False):
        ...
        rows.append({
            ...
            "mean_heldout_mse": attempted["heldout_mse"].mean() if len(attempted) else np.nan,
            "std_heldout_mse": attempted["heldout_mse"].std() if len(attempted) else np.nan,
